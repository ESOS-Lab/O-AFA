...
static void init_stripe(struct stripe_head *sh, sector_t sector, int previous)
{
	...
	for (i = sh->disks; i--; ) {
		...
		if (dev->toread || dev->read || dev->towrite || dev->written ||
			...
			printk(KERN_ERR "sector=%lld i=%d %p %p %p %p %d\n",
			       (unsigned long long)sh->sector, i, dev->toread,
			       dev->read, dev->towrite, dev->written,
			       test_bit(R5_LOCKED, &dev->flags));
			printk(KERN_ERR "Prev Sector=%lld\n",temp_sector);
			WARN_ON(1);
		}
		dev->flags = 0;
...
static void ops_run_io(struct stripe_head *sh, struct stripe_head_state *s)
{
	...
	raid_epoch_array = kmalloc(sizeof(struct raid_epoch *) * disks, GFP_NOIO);
	bdisk_num_array = kmalloc(sizeof(unsigned long long) * disks, GFP_NOIO);	
	barrier_array = kmalloc(sizeof(unsigned long long) * disks, GFP_NOIO);	
	
	if (!raid_epoch_array || !bdisk_num_array || !barrier_array) {
	...
	for (i = disks; i--; ) {
		...
		if (rdev) {
			...
			/* SW Modified : Inherit Task and Raid Epoch */
			if (rw & REQ_ORDERED && i != sh->pd_idx) {
        			bi->shadow_pid = sh->dev[i].written->shadow_pid;
        			bi->raid_epoch = sh->dev[i].written->raid_epoch;
        			bi->task = sh->dev[i].written->raid_epoch->task;
			}
	...
	for (i = disks; i--; ) {
		if (barrier_array[i]) {
			for (k = disks; k--; ) {
				struct bio *bi;
				struct md_rdev *rdev;
				if((bdisk_num_array[i] >> k) & 1)
					continue;
				rdev = rcu_dereference(conf->disks[i].rdev);
				bi = bio_alloc_mddev(GFP_NOIO, 0, mddev);
				bi->bi_private = sh;
				bi->bi_bdev = rdev->bdev;
				bi->bi_rw = REQ_WRITE | REQ_SYNC | REQ_ORDERED | REQ_BARRIER;
				bi->raid_dispatch = 1;
				bi->raid_disk_num = i;
				bi->shadow_pid = barrier_array[i];
				bi->bi_end_io = raid5_end_dbarrier_request;
				bi->raid_epoch = raid_epoch_array[i];
				atomic_inc(&bi->raid_epoch->dbarrier_count);
				atomic_inc(&bi->raid_epoch->e_count);
				atomic_set(&bi->dispatch_check, 0);
				atomic_set(&bi->dbarrier_check, 1);
				atomic_inc(&rdev->nr_pending);
				atomic_inc(&rdev->nr_pending);
				//printk(KERN_INFO "[RAID EPOCH] (%s) [C_BARRIER] "
				//			"PID : %d %llu : %d " 
				//			"Epoch : %p"
				//			"E_Count : %d\n",
			        //	__func__, bi->raid_epoch->task->pid
				//	, (unsigned long long)sh->sector, 
        			//	k, bi->raid_epoch, atomic_read(&bi->raid_epoch->e_count));
				bi->obi = NULL;
				generic_make_request(bi);
				rdev_dec_pending(rdev, conf->mddev);
			}
...
/* SW Modified : Wake Up Page for PG_dispatch */
{
	struct stripe_head *sh;
	...
	while (req_bio) {
		...
		if (bio->bi_end_io == raid5_end_dbarrier_request && bio->raid_epoch
			...
			if(atomic_dec_and_test(&bio->raid_epoch->dbarrier_count)
				...
				for (i = sh->disks; i--;) {
					...
					dev = &sh->dev[i];                    
       					wbi = dev->written;
					if (wbi && wbi->raid_epoch == bio->raid_epoch 
						&& atomic_read(&dev->req.dispatch_check)) { 
						//printk(KERN_INFO "(%s) %lld : %d\n",
						//	__func__,sh->sector,i);        
        					struct raid_epoch *raid_epoch = wbi->raid_epoch;
        					if (atomic_dec_and_test(&raid_epoch->e_count)){
							printk(KERN_INFO "[DbarrierE] %p %lld : %d\n"
									,raid_epoch,sh->sector,i);
							printk(KERN_INFO "[DbarrierE] (%s) PID : %d "
								"Free RAID EPOCH : %p"
								"%lld : %d\n"
								,__func__,raid_epoch->task->pid
								,raid_epoch
								,sh->sector,i);
                					mempool_free(raid_epoch, 
								mddev->raid_epoch_pool);
        					}
						else {
							printk(KERN_INFO "[DbarrierE] (%s)"
									" PID : %d Epoch %p "
									"Epoch Count : %d\n"
									,__func__
									,raid_epoch->task->pid
			...
			/* Need to check the case in which same sector has double wbi */
        		...
        		if (atomic_dec_and_test(&raid_epoch->e_count)){
				printk(KERN_INFO "[Dbarrier1] %p %lld : %d\n"
							,raid_epoch, sh->sector, bio->raid_disk_num);
				printk(KERN_INFO "[Dbarrier1] (%s) PID : %d "
					"Free RAID EPOCH : %p"
					"%lld : %d\n"
					,__func__,raid_epoch->task->pid
					,raid_epoch
					,sh->sector,bio->raid_disk_num);
                		mempool_free(raid_epoch, mddev->raid_epoch_pool);
        		}
			else {
				printk(KERN_INFO "[Dbarrier1] (%s)"
						" PID : %d Epoch %p "
						"Epoch Count : %d\n"
						,__func__
						,raid_epoch->task->pid
		...
		if (bio->raid_epoch 
			...
			/* SW Modified : Track Dispatched Page using ops_run_biodrain routine */
					...
					wbi = r5_next_bio(wbi, dev->sector);
                		...
                		if (atomic_dec_and_test(&raid_epoch->e_count)) {
					printk(KERN_INFO "[Data] %p %lld : %d\n",
						raid_epoch,sh->sector,bio->raid_disk_num);
					printk(KERN_INFO "[Data] (%s) PID : %d "
						"Free RAID EPOCH : %p"
						"%lld : %d\n"
						,__func__,raid_epoch->task->pid
						,raid_epoch
			...
			for (i = sh->disks; i--;) {
				...
				if (wbi && wbi->raid_epoch 
					...
					//	__func__,sh->sector,i);        
        				struct raid_epoch *raid_epoch = wbi->raid_epoch;
        				if (atomic_dec_and_test(&raid_epoch->e_count)) {
						printk(KERN_INFO "[Parity] %p %lld : %d\n",
							raid_epoch,sh->sector,i);
						printk(KERN_INFO "[Parity] (%s) PID : %d "
							"Free RAID EPOCH : %p"
							"%lld : %d\n"
							,__func__,raid_epoch->task->pid
							,raid_epoch
...
void raid5_end_dbarrier_request(struct bio *bi, int error)
{
	...
	int i, disks;
	struct bio *wbi = NULL;
	struct r5dev *dev = NULL, *pdev = NULL;
	sh = bi->bi_private;
	conf = sh->raid_conf;
	mddev = conf->mddev;
	disks = sh->disks;
		...
		if(atomic_dec_and_test(&bi->raid_epoch->dbarrier_count)
			&& atomic_read(&pdev->req.dispatch_check)) {
        		for (i = sh->disks; i--;) {
                		...
                		dev = &sh->dev[i];
                		wbi = dev->written;
				if (wbi && wbi->raid_epoch == bi->raid_epoch
					&& atomic_read(&dev->req.dispatch_check)) {
					//printk(KERN_INFO "(%s) %lld : %d\n",__func__
					//	,sh->sector,i);
					struct raid_epoch *raid_epoch = wbi->raid_epoch;
...
static void raid5_end_fs_dbarrier(struct bio *bi, int error)
{
	...
	//	printk(KERN_INFO "[RAID Layer] (%s) Free bi : %p\n",__func__,bi);
	bio_put(bi);
	rdev_dec_pending(rdev, conf->mddev);
	return;
}
...
void raid5_end_write_request(struct bio *bi, int error)
{
	...
	if (bi->raid_epoch
	        ...
	        struct raid_epoch *raid_epoch = bi->raid_epoch;
		if (wbi && atomic_read(&pdev->req.dispatch_check) 
			&& atomic_read(&bi->raid_epoch->dbarrier_count)) {
			if (atomic_dec_and_test(&raid_epoch->e_count)) {
                  		printk(KERN_INFO "[Data] (%s) PID : %d "                                          				"Free RAID EPOCH : %p\n"             
        				,__func__,raid_epoch->task->pid 
        				,raid_epoch);                        
		...
		&& !atomic_cmpxchg(&bi->dispatch_check, 0, 1)) {
        	for (i = sh->disks; i--;) {                                                
                	...
                	if (wbi && wbi->raid_epoch                                         
                        	...
                        	struct raid_epoch *raid_epoch = wbi->raid_epoch;           
                        	if (atomic_dec_and_test(&raid_epoch->e_count)) {      
                        	        printk(KERN_INFO "[Parity] (%s) PID : %d "                                          				"Free RAID EPOCH : %p\n"             
        						,__func__,raid_epoch->task->pid 
        						,raid_epoch);                        
					mempool_free(raid_epoch,mddev->raid_epoch_pool);   
				}
	...
	if (!test_and_clear_bit(R5_DOUBLE_LOCKED, &sh->dev[bi->raid_disk_num].flags)) {
		clear_bit(R5_LOCKED, &sh->dev[bi->raid_disk_num].flags);
	}
...
static void raid5_build_block(struct stripe_head *sh, int i, int previous)
{
	...
	bio_init(&dev->req);
	dev->req.raid_disk_num = i; /* SW Modified */
	dev->req.raid_dispatch = 1;
	...
	bio_init(&dev->rreq);
	dev->rreq.raid_disk_num = i; /* SW Modified */
	dev->rreq.raid_dispatch = 1;
	...
	*bip = bi;
	...
	if (forwrite) {
		...
		if (sector >= sh->dev[dd_idx].sector + STRIPE_SECTORS)
			...
			printk(KERN_ERR "[RAID EPOCH] (%s) %llu : %d Sector is Not aligned!\n"
				,__func__
...
static void raid5_barrier_request (struct mddev *mddev, struct r5conf *conf, struct bio *bio)
{
	...
	for (i = disks; i--; ) {
		...
		rdev = rcu_dereference(conf->disks[i].rdev);
		bi = bio_alloc_mddev(GFP_NOIO, 0, mddev);
		// printk(KERN_INFO "[RAID Layer] (%s) Alloc bi : %p\n",__func__,bi);
		bi->bi_private = mddev;
		bi->bi_bdev = rdev->bdev;
		bi->bi_rw = REQ_WRITE | REQ_SYNC | REQ_ORDERED | REQ_BARRIER;
		bi->raid_dispatch = 0;
		...
		bi->raid_epoch = NULL;
		atomic_set(&bi->dispatch_check, 0);
		atomic_set(&bi->dbarrier_check, 2);
		bi->obi = bio;
		...
		if (sh) {
			...
			if (bi->bi_rw & REQ_ORDERED) {
				...
				if (!current->__raid_epoch) {
					...
					if (!raid_epoch) {
						printk(KERN_ERR "[RAID EPOCH] (%s) Epoch Alloc"
								" Failed!\n"
								,__func__);
						return;
					}
				...
				bi->raid_epoch = current->__raid_epoch;
				spin_lock_irqsave(&bi->raid_epoch->raid_epoch_lock, flags);
				...
				if (bi->bi_rw & REQ_BARRIER) {
					raid_epoch = current->__raid_epoch;
				...
				spin_unlock_irqrestore(&bi->raid_epoch->raid_epoch_lock, flags);
			}
			set_bit(STRIPE_HANDLE, &sh->state);
			clear_bit(STRIPE_DELAYED, &sh->state);
